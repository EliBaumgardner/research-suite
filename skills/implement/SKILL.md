...
- the proposal's path, and the path of every source report its `Sources:` line names under `.claude/context/GeminiAnalysis/Reviewed/` (ProgramAudits or ResearchReports), plus any `.claude/notes/ongoing-issues.md` entries it cites;
...
- the instruction to read `.claude/CLAUDE.md` in full — the model invariants, Directory Responsibilities, *How to Systematically Solve Problems* and the Key Design Rules;
...
4. **The plan is sound** — each step's changes, behaviour delta and real-time safety argument hold against the code now; every step leaves the tree building; a traversal change carries a parity shape for `TraversalTests.cpp`, walked both as nodes and as modulators (*every walk is the same traversal*).
...
2. **Make the changes** — extract, inline and rename through `refactor.encap` / `refactor.decap` / `refactor.replace`, and multi-site mechanical rewrites through `refactor.rewrite`, as the step names them; never by hand. Every new `.cpp` is registered in `CMakeLists.txt`. The Key Design Rules bind every line: no comments, no ternaries, braces on every block, no wrappers, enums for named states, no accessors.
3. **Verify** — do what the step's *Verification* says: build `SequenceTree_Standalone`, build and run `SequenceTree_Tests` and `SequenceTree_GraphTests` (`ctest --output-on-failure` in `cmake-build-debug`), add the Catch2 test the step names, and run `.claude/gates/design-rules.sh` and `.claude/gates/readability.sh --file <path>` on every changed file. Build only into `cmake-build-debug`; never configure a new build tree.
...
5. **Report the step** — what changed (`file:line`), the build and test results as they came out, the manual host check the step lists (which only the owner can run), and the added/removed line tally.
...
- add an `## Implementation Notes` section at the end: per step, what was built, where it differed from the plan and why, and the manual host checks still owed;
- move the file from `Proposals/Unimplemented/` to `Proposals/Implemented/` — it is removed from `Unimplemented/`, not copied;
- if the proposal resolves an entry in `.claude/notes/ongoing-issues.md`, mark it resolved there, citing the proposal.
...
Under headers: the verifier's verdict and any amendments, each step's result, the tests and gates as they ran, the manual host checks the owner still needs to do, where the proposal was filed, and the total added/removed line tally.
